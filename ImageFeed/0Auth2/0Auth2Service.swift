...
final class OAuth2Service {
    ...
    func fetchOAuthToken(
        ...
        guard let url = URL(string: "https://unsplash.com/oauth/token") else {
            let errorMessage =
                "[fetchOAuthToken]: NetworkError - can't create URL"
            print(errorMessage)
        ...
        print(
            "HTTP request configured with method POST and Content-Type application/json"
        )
        ...
        do {
            ...
            let errorMessage =
                "[fetchOAuthToken]: - \(error.localizedDescription)"
            print(errorMessage)
        ...
        let task = URLSession.shared.objectTask(for: request) {
            ...
            if lastCode != code {
                print(
                    "Received response for outdated code: \(code). Ignoring result."
                )
